      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              {['نام', 'موبایل', 'ایمیل', 'وضعیت', 'عملیات'].map(h => (
              ...
              <tr key={c.id} className="hover:bg-gray-50">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full gradient-header flex items-center justify-center text-white text-sm">
                      {c.fullName[0]}
                    </div>
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-1 rounded-lg text-xs ${c.isActive ? 'status-confirmed' : 'status-cancelled'}`}>
        ...
        {clients.length === 0 && !loading && (
          <div className="text-center py-12 text-gray-400">مشتری‌ای یافت نشد</div>
        )}
      </div>
    </div>
  )
